...
**Legacy data**: rows written before this change (or by a rolled-back backend) keep `frontality`/`sharpness` `NULL` forever — there is no backfill/rescan pass for existing `face_detections` rows (re-running detection would rebuild face rows entirely, a much bigger and riskier operation than this feature warrants; tracked as an optional future ops item, not implemented). New photos indexed after upgrading benefit progressively as they're detected by the updated `mlserver`; the library's cover-selection quality improves gradually rather than all at once.
