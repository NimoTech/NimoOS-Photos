...
func TestPruneStaging_RemovesOldFiles(t *testing.T) {
	...
	oldInfo := old + ".info"
	if err := os.WriteFile(old, []byte("x"), 0600); err != nil { t.Fatal(err) }
	if err := os.WriteFile(oldInfo, []byte("{}"), 0600); err != nil { t.Fatal(err) }
	...
	if err := os.WriteFile(fresh, []byte("x"), 0600); err != nil { t.Fatal(err) }
	if err := os.WriteFile(freshInfo, []byte("{}"), 0600); err != nil { t.Fatal(err) }
